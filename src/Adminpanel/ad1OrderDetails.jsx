...
// const confirmDates = async () => {
//   // First check if order status is Pending Client Confirmation
//   if (safeOrder.order_status === 'Pending Client Confirmation') {
//     toast.error("Please change the order status from 'Pending Client Confirmation' before confirming date changes.");
//     return;
//   }
...
//     const printingCost = currentProduct?.printingCost || 0;
//     const mountingCost = currentProduct?.mountingCost || 0;
    
//     // Calculate individual totals
//     const bookingTotal = productPrice * totalDays;
...
//     // Calculate new totals for the entire order
//     const calculationResult = calculateOrderTotals(updatedProducts, safeOrder.client?.gstPercentage || 18);

//     // Prepare complete update data with ALL order information
//     const updatePayload = {
//       products: updatedProducts,
//       client: {
//         userId: safeOrder.client.userId,
//         name: safeOrder.client.name,
//         email: safeOrder.client.email,
//         contact: safeOrder.client.contact,
//         company: safeOrder.client.company,
//         address: safeOrder.client.address,
//         pincode: safeOrder.client.pincode,
//         state: safeOrder.client.state,
//         city: safeOrder.client.city,
//         paidAmount: safeOrder.client.paidAmount || [],
//         gstPercentage: safeOrder.client.gstPercentage || 18,
//         totalAmount: calculationResult.totalBookingAmount,
//         overAllTotalAmount: calculationResult.overallBase,
//         gstAmount: calculationResult.gstAmount,
//         totalAmountWithGST: calculationResult.totalWithGST,
//         balanceAmount: calculationResult.totalWithGST - getTotalPaid(safeOrder.client.paidAmount)
//       },
//       overAllTotalAmount: calculationResult.overallBase,
//       gstPercentage: safeOrder.client.gstPercentage || 18,
//       gstAmount: calculationResult.gstAmount,
//       totalAmountWithGST: calculationResult.totalWithGST,
//       handled_by: safeOrder.handled_by,
...
//         gstAmount: calculationResult.gstAmount,
//         totalAmountWithGST: calculationResult.totalWithGST,
//         balanceAmount: calculationResult.totalWithGST - getTotalPaid(prev.client.paidAmount)
//       },
//       overAllTotalAmount: calculationResult.overallBase,
//       gstAmount: calculationResult.gstAmount,
//       totalAmountWithGST: calculationResult.totalWithGST,
...
// const getTotalPaid = (paidAmount) => {
//   if (!paidAmount) return 0;
//   if (Array.isArray(paidAmount)) {
//     return paidAmount.reduce((sum, p) => sum + (p.amount || 0), 0);
//   }
//   return typeof paidAmount === 'number' ? paidAmount : 0;
// }; 
...
//     ₹{product.price || 0}/day
//   </div>
//   {product.booking?.totalPrice > 0 && (
//     <div style={{ fontSize: '12px', color: '#666' }}>
//       Total: {formatIndianCurrency(product.booking.totalPrice, true)}
//     </div>
//   )}
//   {(product.printingCost > 0 || product.mountingCost > 0) && (
//     <div style={{ fontSize: '11px', color: '#888' }}>
//       Print: ₹{product.printingCost || 0} | Mount: ₹{product.mountingCost || 0}
//     </div>
//   )}
// </td>
...
// const calculateDetailedTotals = (products, gstPercentage = 18) => {
//   let bookingTotal = 0;
//   let printingTotal = 0;
//   let mountingTotal = 0;
...
//   products.forEach(product => {
//     if (!product.deleted) {
//       bookingTotal += product.booking?.totalPrice || 0;
//       printingTotal += product.printingCost || 0;
//       mountingTotal += product.mountingCost || 0;
//     }
//   });

//   const overallBase = bookingTotal + printingTotal + mountingTotal;
//   const gstAmount = Math.floor(overallBase * (gstPercentage / 100));
//   const totalWithGST = overallBase + gstAmount;
...
// // Use this in your component to display totals
// const totals = calculateDetailedTotals(safeOrder.products, safeOrder.client?.gstPercentage || 18);
...
//  <div className="order-clientDetailSection">
//     <div className="order-clientDetailHeading">Payment (Base Amount)</div>
//     <input
//       type="text"
//       placeholder="Enter Payment"
//       className="order-clientDetailsInput"
//       value={formatIndianCurrency(safeOrder.client?.totalAmount || 0, true)}
//       readOnly
//     ></input>
//   </div>
  
//   <div className="order-clientDetailSection">
//     <div className="order-clientDetailHeading">Printing Cost</div>
//     <input
//       type="text"
//       placeholder="Printing Cost"
//       className="order-clientDetailsInput"
//       value={formatIndianCurrency(safeOrder.products?.reduce((sum, p) => sum + (p.printingCost || 0), 0) || 0, true)}
//       readOnly
//     ></input>
//   </div>
  
//   <div className="order-clientDetailSection">
//     <div className="order-clientDetailHeading">Mounting Cost</div>
//     <input
//       type="text"
//       placeholder="Mounting Cost"
//       className="order-clientDetailsInput"
//       value={formatIndianCurrency(safeOrder.products?.reduce((sum, p) => sum + (p.mountingCost || 0), 0) || 0, true)}
//       readOnly
//     ></input>
//   </div>
  
//   <div className="order-clientDetailSection">
//     <div className="order-clientDetailHeading">Overall Base (Excl. GST)</div>
//     <input
//       type="text"
//       className="order-clientDetailsInput"
//       value={formatIndianCurrency(safeOrder.client?.overAllTotalAmount || 0, true)}
//       readOnly
//     ></input>
//   </div>
  
//   <div className="order-clientDetailSection">
//     <div className="order-clientDetailHeading">GST @{safeOrder.client?.gstPercentage || 18}%</div>
//     <input
//       type="text"
//       className="order-clientDetailsInput"
//       value={formatIndianCurrency(safeOrder.client?.gstAmount || 0, true)}
//       readOnly
//     ></input>
//   </div>
  
//   <div className="order-clientDetailSection">
//     <div className="order-clientDetailHeading">Total (Incl. GST)</div>
//     <input
//       type="text"
//       className="order-clientDetailsInput"
//       style={{ fontWeight: 'bold', color: '#2ecc71' }}
//       value={formatIndianCurrency(safeOrder.client?.totalAmountWithGST || 0, true)}
//       readOnly
//     ></input>
//   </div>
...
// Helper function to calculate individual product totals with GST
...
function OrderDetails({ order, onOrderUpdate }) {
  ...
  const confirmDelete = async () => {
    try {
      ...
      if (result.status) {
        ...
        // Calculate new totals using the helper function
        const totals = calculateOrderTotals(updatedProducts, safeOrder.client?.gstPercentage || 18);
        const totalPaid = getTotalPaid(safeOrder.client?.paidAmount);
        ...
        const updatedOrder = {
          ...
          client: {
            ...safeOrder.client,
            totalAmount: totals.totalBookingAmount,
            overAllTotalAmount: totals.overallBase,
            gstAmount: totals.gstAmount,
            totalAmountWithGST: totals.totalWithGST,
            balanceAmount: Math.max(totals.totalWithGST - totalPaid, 0),
          },
          overAllTotalAmount: totals.overallBase,
          gstAmount: totals.gstAmount,
          totalAmountWithGST: totals.totalWithGST,
  ...
  const totals = calculateOrderTotals(safeOrder.products || [], safeOrder.client?.gstPercentage || 18);

  // Payment calculations - use totals.totalWithGST for balance
  const totalPaidAmount = getTotalPaid(safeOrder.client?.paidAmount);
  const [advanceAmount, setAdvanceAmount] = useState(0);
  const [remainingAmount, setRemainingAmount] = useState(
    Math.max(totals.totalWithGST - totalPaidAmount, 0),
  );
  ...
  // Recalculate remaining amount when totals change
  useEffect(() => {
    const newRemaining = Math.max(totals.totalWithGST - totalPaidAmount, 0);
    setRemainingAmount(newRemaining);
  }, [totals.totalWithGST, totalPaidAmount]);
  ...
  const handleAdvanceChange = (e) => {
    ...
    const adv = Number(value);
    setAdvanceAmount(adv);
    setRemainingAmount(Math.max(totals.totalWithGST - (totalPaidAmount + adv), 0));
  };
      ...
      if (data.status) {
        ...
        setOrderData((prev) => ({
          ...
          client: {
            ...prev.client,
            paidAmount: updatedPaid,
            balanceAmount: Math.max(totals.totalWithGST - getTotalPaid(updatedPaid), 0),
          },
          last_edited: new Date().toISOString(),
        }));
  ...
  */
  ...
  const currentProduct = safeOrder.products[activeProductIndex] || {};
  const pricePerDay = currentProduct.price || 0;
  const productPrintingCost = currentProduct.printingCost || 0;
  const productMountingCost = currentProduct.mountingCost || 0;
  ...
  const availableDays =
    selectedDates.start && selectedDates.end
      ...
      : [];
  const totalDays = availableDays.length;
  const bookingTotal = totalDays * pricePerDay;
  
  // Calculate product totals with GST
  const productOverallBase = bookingTotal + productPrintingCost + productMountingCost;
  const productGstAmount = Math.floor(productOverallBase * ((safeOrder.client?.gstPercentage || 18) / 100));
  const productTotalWithGST = productOverallBase + productGstAmount;
    ...
    try {
      ...
      const updatePayload = {
        ...
        client: {
          ...
          gstAmount: calculationResult.gstAmount,
          totalAmountWithGST: calculationResult.totalWithGST,
          balanceAmount: Math.max(calculationResult.totalWithGST - totalPaid, 0)
        },
        overAllTotalAmount: calculationResult.overallBase,
        gstPercentage: safeOrder.client.gstPercentage || 18,
        gstAmount: calculationResult.gstAmount,
      ...
      setOrderData((prev) => ({
        ...
        client: {
          ...
          gstAmount: calculationResult.gstAmount,
          totalAmountWithGST: calculationResult.totalWithGST,
          balanceAmount: Math.max(calculationResult.totalWithGST - getTotalPaid(prev.client.paidAmount), 0)
        },
        overAllTotalAmount: calculationResult.overallBase,
        gstAmount: calculationResult.gstAmount,
        totalAmountWithGST: calculationResult.totalWithGST,
  ...
  const saveCancelReason = async () => {
    ...
    try {
      ...
      if (res?.status === true) {
        ...
        // Calculate totals after cancellation (should be 0)
        const totals = calculateOrderTotals(cancelledProducts, safeOrder.client?.gstPercentage || 18);
  ...
  const formattedOverallBase = formatIndianCurrency(productOverallBase, true);
  const formattedGstAmount = formatIndianCurrency(productGstAmount, true);
  const formattedTotalWithGST = formatIndianCurrency(productTotalWithGST, true);

  // Render product row
  const renderProductRow = (product, index) => {
    const isDeleted = product.deleted;
    const rowClass = isDeleted ? "deleted-product-row" : "";
    const productTotals = calculateProductTotals(product, safeOrder.client?.gstPercentage || 18);
    ...
    return (
      ...
      >
        ...
        <td>
          ...
          {(product.printingCost > 0 || product.mountingCost > 0) && (
            <div style={{ fontSize: '11px', color: '#888' }}>
              Print: {formatIndianCurrency(product.printingCost || 0, true)} | 
              Mount: {formatIndianCurrency(product.mountingCost || 0, true)}
            </div>
          )}
        </td>
        <td>
        ...
        <>
              ...
              <div className="order-manageClientInfoLeft">
                ...
                <div className="order-clientDetailSection">
                  ...
                  <input
                    ...
                    placeholder="Enter Payment"
                    className="order-clientDetailsInput"
                    value={formatIndianCurrency(totals.totalBookingAmount, true)}
                    readOnly
                  ></input>
                </div>
                ...
                <div className="order-clientDetailSection">
                  ...
                  <input
                    ...
                    placeholder="Printing Cost"
                    className="order-clientDetailsInput"
                    value={formatIndianCurrency(totals.totalPrintingCost, true)}
                    readOnly
                  ></input>
                </div>
                ...
                <div className="order-clientDetailSection">
                  ...
                  <input
                    ...
                    placeholder="Mounting Cost"
                    className="order-clientDetailsInput"
                    value={formatIndianCurrency(totals.totalMountingCost, true)}
                    readOnly
                  ></input>
                </div>
                ...
                <div className="order-clientDetailSection">
                  ...
                  <input
                    type="text"
                    className="order-clientDetailsInput"
                    value={formatIndianCurrency(totals.overallBase, true)}
                    readOnly
                  ></input>
                </div>
                ...
                <div className="order-clientDetailSection">
                  ...
                  <input
                    type="text"
                    className="order-clientDetailsInput"
                    value={formatIndianCurrency(totals.gstAmount, true)}
                    readOnly
                  ></input>
                </div>
                ...
                <div className="order-clientDetailSection">
                  ...
                  <input
                    ...
                    className="order-clientDetailsInput"
                    style={{ fontWeight: 'bold', color: '#2ecc71' }}
                    value={formatIndianCurrency(totals.totalWithGST, true)}
                    readOnly
                  ></input>
                </div>
                ...
                <div className="order-clientDetailSection">
                  <div className="order-clientDetailHeading">
                    ...
                    <span className="tooltip-icon-wrapper">
                      ...
                      <div className="tooltip-content">
                          ...
                          <table className="payment-table">
                            ...
                            <tbody>
                              {paymentHistory.map((p, index) => (
                                <tr key={index}>
                                  <td>{index + 1}</td>
                                  <td>{formatIndianCurrency(p.amount, true)}</td>
                                  <td>{formatIndianDateTime(p.paidAt)}</td>
                                </tr>
                              ))}
                            </tbody>
                        ...
                        <div className="tooltip-summary">
                          <div>
                            <strong>Total Paid:</strong> {formatIndianCurrency(totalPaidAmount, true)}
                          </div>
                          <div>
                            <strong>Remaining:</strong> {formatIndianCurrency(remainingAmount, true)}
                          </div>
                        </div>
                      </div>
                    </span>
                  ...
                  <input
                    type="text"
                    className="order-clientDetailsInput"
                    value={formatIndianCurrency(remainingAmount, true)}
                    readOnly
                  />
                </div>
          ...
          <div className="order-manageClientSection adminOrder_productSection">
            ...
            <div className="product-summary-stats">
              <span className="active-products-count">
                <i className="fas fa-check-circle"></i> Active Products:{" "}
                {totals.activeProductsCount}
              </span>
              {totals.deletedProductsCount > 0 && (
                <span className="deleted-products-count">
                  <i className="fas fa-trash"></i> Deleted Products:{" "}
                  {totals.deletedProductsCount}
                </span>
              )}
              <span className="total-products-count">
                <i className="fas fa-boxes"></i> Total Products:{" "}
                        ...
                        <div className='d-flex' style={{ justifyContent: 'space-between', flexWrap: 'wrap' }}>
                          <div style={{ width: '100%' }}>
                            <div className='ManageProdDetails'>
                              ...
                              <div> : </div>
                              <div className='ManageProdRightContent' style={{ fontWeight: '600' }}>{formatIndianCurrency(pricePerDay, true)} / Per Day</div>
                            </div>
                            <div className='ManageProdDetails'>
                            ...
                            <div className='ManageProdDetails'>
                              ...
                              <div> : </div>
                              <div className='ManageProdRightContent' style={{ fontWeight: '600' }}>{formatIndianCurrency(bookingTotal, true)}</div>
                            </div>
                            <div className='ManageProdDetails'>
                              <div className='ManageProdLeftHeading'>Printing Cost </div>
                              <div> : </div>
                              <div className='ManageProdRightContent' style={{ fontWeight: '600' }}>{formatIndianCurrency(productPrintingCost, true)}</div>
                            </div>
                            <div className='ManageProdDetails'>
                              <div className='ManageProdLeftHeading'>Mounting Cost </div>
                              <div> : </div>
                              <div className='ManageProdRightContent' style={{ fontWeight: '600' }}>{formatIndianCurrency(productMountingCost, true)}</div>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className='manageClientSection' style={{ marginTop: '20px', borderTop: '2px solid #ddd', paddingTop: '15px' }}>
                        ...
                        <div className='d-flex' style={{ justifyContent: 'space-between', flexWrap: 'wrap' }}>
                          <div style={{ width: '100%' }}>
                            ...
                            <div className='ManageProdDetails'>
                              <div className='ManageProdLeftHeading'>GST @ {safeOrder.client?.gstPercentage || 18}% </div>
                              <div> : </div>
            ...
            <div className="admin-order-pricing">
              ...
              <div className="admin-orderContent">
                ...
                <div
                  ...
                  style={{ width: "auto" }}
                >
                  {formatIndianCurrency(totals.totalWithGST, true)}
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="confirm-order-dialog">
                ...
                <div className="dialog-body">
                  ...
                  <div className="order-summary">
                    ...
                    <p>
                      <strong>Total Amount:</strong> {formatIndianCurrency(totals.totalWithGST, true)}
                    </p>
                    <p>
                      <strong>Dates:</strong>{" "}
                      {safeOrder.products?.length > 0
